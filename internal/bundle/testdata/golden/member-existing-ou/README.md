...
| Trusted principal | `arn:aws:iam::222222222222:role/automat-runner` (one role, not the whole account) |
| Organization | o-exampleorgid, management account 111111111111 |
| Target OU | ou-exam-research1 |
| Role to create | `automat-vendor` in the management account |
...
- **It cannot place an account anywhere else.** `organizations:MoveAccount` is
  restricted by resource to OU ou-exam-research1 and its descendants as the destination. An
...
3. Reply to research-it@example.edu with the role ARN, which is
   `arn:aws:iam::111111111111:role/automat-vendor` unless you renamed it.
...
This bundle already names one role rather than the whole account, which is the
narrower of the two forms automat supports.
