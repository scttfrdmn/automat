...
Documented in full in `docs/conventions.md`: the account tags `vend` applies, the SCP
naming scheme, and the single owner tag (`automat:managed-by=automat`) every ownership
check in the codebase reads. External systems may adopt these; automat depends on nothing
external in return.
...
- A remote manifest mirror (`s3://automat-evidence-<acct>/manifests/…` plus a
  management-side copy) is named in DESIGN §11's own compensating-control argument but not
  built — see `docs/conventions.md`'s own note on what remains future.
