...
**Decided by the maintainer on Q14; the rename has landed, the fields have not.** `profile/v1` is now `environment-profile/v1` because "profile" named three unrelated documents (the per-vend input, obligation profiles, and the classification profiles below) plus a fourth automat cannot rename — the **AWS credential profile**. `vend`'s input flag is `--environment-profile`; `--profile` stays reserved for the AWS sense. DESIGN §7a states the taxonomy, `schema/CHANGELOG.md` records the rename field by field, and it moved every record hash, so the golden manifest was regenerated. **This is why the golden file exists:** the diff is the field name and the hashes that must follow it, and nothing else.
...
- **`verify` warns when an environment or obligation profile's `review_by` has lapsed** (DESIGN §11a, §12). A warning and not a failure: the account is exactly as compliant as it was the day before, and what has expired is anyone's assurance that the document describing it still reads policy correctly. A hard failure would also make `verify` unusable in the cron job it exists for, and an unusable check gets disabled. This is the citation-freshness audit rule expressed as data rather than as a ritual — the failure mode it catches is silent and confident, since a superseded citation renders exactly as well as a current one.
